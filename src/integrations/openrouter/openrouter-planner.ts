...
import { Planner, PlannerEvidenceError, PlannerInput, PlannerResult } from "../../harness/agent/planning/planner"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function*() {
    ...
    const plan = Effect.fn("Planner.plan")(function*(input: PlannerInput) {
      ...
      return new PlannerResult({ plannerVersion, model: config.openRouterModel, latencyMs: Number(end - start) / 1_000_000, inputTokens: response.usage.inputTokens.total, outputTokens: response.usage.outputTokens.total, recommendation: response.value })
