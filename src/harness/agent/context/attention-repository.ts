  ...
  Effect.gen(function*() {
    ...
    return AttentionRepository.of({
      putIfAbsent: Effect.fn("AttentionRepository.putIfAbsent")(function*(item) {
        ...
        yield* sql`INSERT INTO attention_items (attention_id, detector, dedupe_key, principal_id, kind, payload_json, status, created_at)
          VALUES (${item.attentionId}, ${item.detector}, ${item.dedupeKey}, ${item.principalId}, ${item.kind}, ${JSON.stringify(item.payload)}, ${item.status}, ${item.createdAt})`
        return true
      }),
      get: Effect.fn("AttentionRepository.get")(function*(attentionId) {
        ...
        const row = rows[0]
        return row === undefined ? Option.none() : Option.some(new AttentionItem({ attentionId: row.attention_id, detector: row.detector, dedupeKey: row.dedupe_key, principalId: row.principal_id, kind: row.kind, payload: JSON.parse(row.payload_json), status: row.status, createdAt: row.created_at }))
      }),
      setStatus: (attentionId, status) => Effect.asVoid(sql`UPDATE attention_items SET status = ${status} WHERE attention_id = ${attentionId}`)
    })
  })
)
