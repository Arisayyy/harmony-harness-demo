...
export class GateDenied extends Data.TaggedError("GateDenied")<{
    ...
    return Gate.of({
      evaluate: Effect.fn("Gate.evaluate")(function*(principal, recommendation) {
        ...
        if (recommendation._tag === "EnterWorkflow") {
          for (const scope of ["erp:po:create", "erp:po:cancel", "production:notify", "mail:send"]) {
            if (!hasScope(principal, scope)) missing.add(scope)
          }

          const suppliers = yield* erp.listSuppliersForPart(principal, recommendation.parameters.partId).pipe(
            Effect.mapError(() => new GateDenied({ reasons: ["Unable to verify alternate supplier authorization."] }))
          )
          ...
          if (supplier === undefined || !supplier.approved || !supplier.approvedParts.includes(recommendation.parameters.partId)) {
            return yield* new GateDenied({ reasons: ["Alternate supplier is not approved for the requested part."] })
          }
          ...
          for (const action of recommendation.actions) {
            const required = action._tag === "quality.reallocate-lot"
              ? ["erp:quality:reallocate", "production:notify"]
              : action._tag === "production.notify"
                ? ["production:notify", "mail:send"]
                : ["purchasing:flag-shortage", "mail:send"]
        ...
        return {
          planHash: yield* hashPlan(recommendation),
          assignedApproverId,
          policyReason
        }
      })
