  ...
  Effect.gen(function*() {
    ...
    return ScheduledWorkService.of({
      schedule: Effect.fn("ScheduledWork.schedule")(function*(work) {
        ...
        yield* sql`INSERT OR IGNORE INTO scheduled_work (work_id, run_at, kind, payload_json, status, dedupe_key, created_at) VALUES (${work.workId}, ${work.runAt}, ${work.kind}, ${JSON.stringify(work.payload)}, 'scheduled', ${work.dedupeKey}, ${createdAt})`
      ...
      complete: (workId) => Effect.asVoid(sql`UPDATE scheduled_work SET status = 'complete' WHERE work_id = ${workId}`),
