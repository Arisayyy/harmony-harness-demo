...
export class AuditRepository extends Context.Service<AuditRepository, {
  readonly append: (event: AuditEvent) => Effect.Effect<void, SqlError>
  readonly listRun: (runId: string) => Effect.Effect<ReadonlyArray<AuditEvent>, SqlError>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function*() {
    ...
    return AuditRepository.of({
      append: (event) => Effect.asVoid(sql`INSERT INTO audit_events (event_id, run_id, trace_id, event_type, actor, effective_user_id, occurred_at, evidence_json, data_json) VALUES (${event.eventId}, ${event.runId}, ${event.traceId}, ${event.eventType}, ${event.actor}, ${event.effectiveUserId ?? null}, ${event.occurredAt}, ${JSON.stringify(event.evidence)}, ${JSON.stringify(event.data)})`),
