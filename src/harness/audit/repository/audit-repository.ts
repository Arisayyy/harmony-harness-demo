...
export const layer = Layer.effect(
  ...
  Effect.gen(function*() {
    ...
    return AuditRepository.of({
      append: (event) => Effect.asVoid(sql`INSERT INTO audit_events (
        event_id, run_id, trace_id, event_type, actor, effective_user_id, occurred_at, evidence_json, data_json
      ) VALUES (
        ${event.eventId}, ${event.runId}, ${event.traceId}, ${event.eventType}, ${event.actor}, ${event.effectiveUserId ?? null},
        ${event.occurredAt}, ${JSON.stringify(event.evidence)}, ${JSON.stringify(event.data)}
      )`),
      listRun: Effect.fn("AuditRepository.listRun")(function*(runId) {
        ...
        return rows.map((row) => new AuditEvent({
          eventId: row.event_id,
          runId: row.run_id,
          traceId: row.trace_id,
          eventType: row.event_type,
          actor: row.actor,
          effectiveUserId: row.effective_user_id ?? undefined,
...
export const appendNow = (event: Omit<AuditEvent, "eventId" | "occurredAt">) => Effect.gen(function*() {
  ...
  const clock = yield* BusinessClock
  const occurredAt = yield* clock.now
  yield* repository.append(new AuditEvent({ ...event, eventId: crypto.randomUUID(), occurredAt }))
})
