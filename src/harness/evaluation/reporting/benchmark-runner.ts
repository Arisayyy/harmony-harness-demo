...
const estimateCost = (result: PlannerResult) => result.inputTokens === undefined || result.outputTokens === undefined
  ? undefined
  : (result.inputTokens / 1_000_000) * glmFlashInputPerMillion + (result.outputTokens / 1_000_000) * glmFlashOutputPerMillion

export class BenchmarkRunner extends Context.Service<BenchmarkRunner, {
  readonly live: Effect.Effect<void, unknown>
  readonly replay: Effect.Effect<void, unknown>
}>()("harmony/evaluation/BenchmarkRunner") {}
    ...
    return BenchmarkRunner.of({
      live: Effect.gen(function*() {
        ...
        for (const benchmark of cases) {
          ...
          for (let repetition = 1; repetition <= 3; repetition++) {
            ...
            yield* sql`INSERT INTO benchmark_runs (benchmark_run_id, case_id, fixture_version, model, planner_version, repetition, result_json, latency_ms, input_tokens, output_tokens, estimated_cost, created_at)
              VALUES (${benchmarkRunId}, ${benchmark.id}, ${benchmark.fixtureVersion}, ${result.model}, ${result.plannerVersion}, ${repetition}, ${JSON.stringify({ plannerResult: result, score })}, ${result.latencyMs}, ${result.inputTokens ?? null}, ${result.outputTokens ?? null}, ${estimatedCost ?? null}, ${createdAt})`
      ...
      replay: Effect.gen(function*() {
        ...
        for (const row of rows) {
          ...
          const recommendation = yield* Schema.decodeUnknown(Recommendation)(stored.plannerResult.recommendation)
