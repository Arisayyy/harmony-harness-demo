  ...
  Effect.gen(function*() {
    ...
    return ToolRuntime.of({
      execute: Effect.fn("ToolRuntime.execute")(function*({ tool: name, principal, input, idempotencyKey }) {
        ...
        if (existing[0] !== undefined) return JSON.parse(existing[0].result_json)
        ...
        yield* sql`INSERT INTO tool_idempotency (idempotency_key, tool_name, result_json, created_at)
          VALUES (${idempotencyKey}, ${name}, ${JSON.stringify(encoded)}, ${new Date().toISOString()})`
