...
export class ToolRuntime extends Context.Service<ToolRuntime, {
  readonly execute: (options: { readonly tool: string; readonly principal: Principal; readonly input: unknown; readonly idempotencyKey: string }) => Effect.Effect<unknown, unknown>
  ...
  Effect.gen(function*() {
    ...
    return ToolRuntime.of({
      execute: Effect.fn("ToolRuntime.execute")(function*({ tool: name, principal, input, idempotencyKey }) {
        ...
        if (existing[0] !== undefined) return yield* decodeOutput(JSON.parse(existing[0].result_json))
